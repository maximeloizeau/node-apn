...
Connection.prototype.connect = function () {
	...
	this.initialize().then(function(credentials) {
        ...
        var options = {
            ...
            socksProxyHost: this.options.socksProxyHost,
            socksProxyPort: this.options.socksProxyPort,
            socksProxyVersion: this.options.socksProxyVersion
        };

        createSocket(this, options,
            function (socket) {
                debug("Connection established");

                this.socket = socket;
                socket.on("error",   this.errorOccurred.bind(this, socket));
                socket.on("timeout", this.socketTimeout.bind(this, socket));
                socket.on("data",    this.handleTransmissionError.bind(this, socket));
                socket.on("drain",   this.socketDrained.bind(this, socket, true));
                socket.once("close", this.socketClosed.bind(this, socket));
